...
def fix_db():
    ...
    cursor.execute("UPDATE trabajadores SET password_hash=? WHERE usuario='admin'", (admin_hash,))

    # empleado / empleado123
    emp_hash = bcrypt.hashpw(b'empleado123', bcrypt.gensalt()).decode()
    cursor.execute("UPDATE trabajadores SET password_hash=? WHERE usuario='empleado'", (emp_hash,))
