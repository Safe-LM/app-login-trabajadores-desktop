...
class GeminiVisionMatcher:
    ...
    def __init__(
        self, api_key: Optional[str] = None, database_dir: Optional[Path] = None
    ):
        """
        ...
        if not self.api_key:
            raise ValueError(
                "Se requiere GEMINI_API_KEY (variable de entorno o parámetro)"
            )
    ...
    def _compare_with_gemini(
        self, query_image: np.ndarray, reference_image: np.ndarray, employee_name: str
    ) -> Tuple[bool, float]:
        """
...
Sé muy estricto. Solo marca is_same_person como true si estás muy seguro (confidence > 0.85)."""
            ...
            if response.status_code != 200:
                ...
                try:
                    ...
                    logger.debug(
                        f"Error Gemini API {response.status_code}: {error_msg}"
                    )
                except Exception:
                    logger.debug(
                        f"Error Gemini API {response.status_code}: {error_detail}"
                    )
            ...
            if self._last_gemini_error != str(e)[:50]:
                logger.warning(
                    f"⚠️ Error HTTP Gemini ({self._error_count}/{self._max_errors}): {str(e)[:100]}"
                )
            ...
            if self._error_count >= self._max_errors:
                self._disabled = True
                logger.warning(
                    f"⚠️ Gemini deshabilitado después de {self._max_errors} errores. Usando solo OpenCV."
                )
            return False, 0.0
        except Exception as e:
            self._error_count += 1
            ...
            if self._last_gemini_error != str(e)[:50]:
                logger.warning(
                    f"⚠️ Error comparando con Gemini ({self._error_count}/{self._max_errors}): {str(e)[:100]}"
                )
                self._last_gemini_error = str(e)[:50]
    ...
    def match_photo(
        self, frame: np.ndarray, min_confidence: float = 0.85
        ...
        try:
            ...
            for employee in self.employees_db:
                ...
                if not self._disabled:
                    is_match, confidence = self._compare_with_gemini(
                        query_face, ref_image, employee_name
                    )
            ...
            if best_confidence >= min_confidence and best_employee:
                logger.info(
                    f"✅ Match Gemini: {best_employee.get('nombre')} (confianza: {best_confidence:.3f})"
                )
