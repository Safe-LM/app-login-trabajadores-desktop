...
def extract_images_from_pdf_pymupdf(pdf_path: Path) -> List[Tuple]:
    ...
    try:
        ...
        for page_num in range(len(doc)):
            ...
            print(
                f"  Pagina {page_num + 1}: {len(image_list)} imagenes encontradas (get_images)"
            )
            ...
            for img_index, img in enumerate(image_list):
                try:
                    ...
                    if width > 50 and height > 50:
                        ...
                        print(
                            f"    [OK] Imagen {total_images_found}: {width}x{height} (pagina {page_num+1}, xref={xref})"
                        )
                    else:
                    ...
                    print(
                        f"    [WARNING] Error extrayendo imagen {img_index} de pagina {page_num+1}: {e}"
                    )
                    continue
                                ...
                                if base_image:
                                    ...
                                    if width > 50 and height > 50:
                                        ...
                                        for existing in images:
                                            if (
                                                existing[0] == page_num
                                                and existing[2] == width
                                                and existing[3] == height
                                            ):
                                                is_duplicate = True
                                                break
                                        ...
                                        if not is_duplicate:
                                            images.append(
                                                (page_num, img_pil, width, height)
                                            )
                                            total_images_found += 1
                                            print(
                                                f"    [OK] Imagen {total_images_found} (XObject): {width}x{height} (pagina {page_num+1})"
                                            )
                        except Exception:
                            pass
            except Exception as e:
                print(
                    f"    [INFO] No se pudieron extraer XObjects de pagina {page_num+1}: {e}"
                )
    ...
    for idx, img_data in enumerate(images):
        try:
            ...
            if has_person:
                ...
                print(
                    f"  [OK] Imagen {idx+1}: Contiene persona (confianza: {confidence:.2f}, pagina {page_num+1})"
                )
            else:
                print(
                    f"  [SKIP] Imagen {idx+1}: No contiene persona (pagina {page_num+1})"
                )
...
def save_images_to_database(
    images: List[Tuple], output_dir: Path, employees: List[Dict]
):
    ...
    if len(images) != len(employees):
        ...
        if len(images) < len(employees):
            print(
                f"  [INFO] Faltan {len(employees) - len(images)} fotos. Se guardaran las disponibles."
            )
        else:
            print(
                f"  [INFO] Hay {len(images) - len(employees)} fotos extras. Se guardaran las primeras {len(employees)}."
            )
    ...
    for idx in range(max_employees):
        try:
            ...
            print(
                f"  [OK] Guardada: {output_path.name} -> {nombre} ({zona}, {sucursal}, {puesto})"
            )
            saved_count += 1
    ...
    if employees:
        ...
        for idx in range(min(10, len(images), len(employees))):
            emp = employees[idx]
            print(
                f"  photo_{emp['employee_id']}.jpg -> {emp['nombre']} ({emp['zona']}, {emp['sucursal']})"
            )
...
def main():
    ...
    if len(face_images) < len(employees):
        print(
            f"[WARNING] Solo se encontraron {len(face_images)} imagenes para {len(employees)} empleados"
        )
        print("  Algunos empleados no tendran foto")
    elif len(face_images) > len(employees):
        print(
            f"[INFO] Se encontraron {len(face_images)} imagenes para {len(employees)} empleados"
        )
