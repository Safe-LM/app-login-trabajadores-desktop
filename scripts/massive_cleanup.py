...
def cleanup_and_reseed():
    ...
    if sb:
        try:
            ...
            sb.table("asistencias").delete().neq("id", "00000000-0000-0000-0000-000000000000").execute()
            sb.table("empleados").delete().neq("id", "00000000-0000-0000-0000-000000000000").execute()
    ...
    for e in employees_manifest:
        ...
        if sb:
            ...
            employee_embs = []
            if encs_data:
                indices = [i for i, idx_id in enumerate(encs_data.get('employee_ids', [])) if idx_id == eid]
                if indices:
                    relevant_encs = [encs_data['encodings'][i] for i in indices]
                    avg_emb = np.mean(relevant_encs, axis=0) # Promedio de los embeddings
    ...
    try:
        db.commit()
        logger.info(f"✅ Se han reinsertado {len(employees_manifest)} empleados exitosamente.")
    except Exception as e:
        db.rollback()
