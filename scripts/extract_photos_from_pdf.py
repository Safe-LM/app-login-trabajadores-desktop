                    ...
                    for obj in xObject:
                        if xObject[obj]["/Subtype"] == "/Image":
                            try:
                                ...
                                print(
                                    f"  [WARNING] Error extrayendo imagen de pagina {page_num}: {e}"
                                )
                                continue
    except Exception as e:
        ...
        for page_num in range(len(doc)):
            ...
            for img_index, img in enumerate(image_list):
                try:
                    ...
                    print(
                        f"  [WARNING] Error extrayendo imagen {img_index} de pagina {page_num}: {e}"
                    )
                    continue
...
def save_images_to_database(images: list, output_dir: Path, employees_csv_path: Path):
    ...
    for idx, (page_num, img_pil) in enumerate(images):
        try:
            ...
            print(
                f"  [OK] Guardada: {output_path.name} -> {nombre} (pagina {page_num+1})"
            )
            saved_count += 1
