...
def prepare_training_data_pro(
    ...
    for employee_id, photo_files in sorted(photos_by_employee.items()):
            ...
            try:
                ...
                if processed is not None:
                    ...
                    M = cv2.getRotationMatrix2D(
                        (w / 2, h / 2), random.uniform(-10, 10), 1.0
                    )
                    ...
                    print(
                        f"  [OK] {photo_file.name} + 2 variaciones (confianza YOLO: {conf:.2f})"
                    )
        ...
        print(
            f"  Total imágenes para este empleado: {employee_info[label_idx]['num_fotos']}\n"
        )
...
def train_model_professional(
    ...
    if not use_stratify:
        print(
            f"[WARNING] Algunas clases tienen solo 1 muestra. Usando split sin estratificación."
        )
    ...
    for epoch in range(epochs):
        ...
        if val_acc > best_val_acc:
            ...
            if patience_counter >= patience:
                print(
                    f"  [STOP] Early stopping activado (sin mejora en {patience} épocas)"
                )
                break
    ...
    print(
        f"[OK] Entrenamiento completado. Mejor precisión de validación: {best_val_acc:.2f}%"
    )
    print()
...
def save_model_professional(model, employee_info: Dict, output_dir: Path):
    ...
    with open(info_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            f"Técnicas: Data Augmentation Avanzado, Label Smoothing, Gradient Clipping\n\n"
        )
...
def main():
    ...
    try:
        # Preparar datos
        images, labels, employee_info = prepare_training_data_pro(
