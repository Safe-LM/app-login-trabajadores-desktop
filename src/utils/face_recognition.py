...
def _load_face_recognition_module():
    ...
    try:
        ...
        app_web_path = (
            Path(__file__).parent.parent.parent / "app_loginTrabajadores" / "backend"
        )
            ...
            try:
                ...
                if not ds.embeddings_empleados:
                    ...
                    if ds.embeddings_empleados:
                        print(
                            f"✅ {len(ds.embeddings_empleados)} embedding(s) cargados"
                        )
                    else:
                        print(
                            "⚠️ No hay embeddings guardados. Ejecuta registrar_fotos.py primero."
                        )
            except Exception as e:
...
def reconocer_desde_frame(
    frame: np.ndarray, trabajador_id: int = None, embedding_idx: int = None
):
...
def reconocer_automatico(frame: np.ndarray):
    ...
    try:
        ...
        if not hasattr(ds, "embeddings_empleados") or len(ds.embeddings_empleados) == 0:
            print(
                "[WARNING] No hay embeddings cargados. Ejecuta registrar_fotos.py primero."
            )
            return False, 0.0, None

        # Usar sistema mejorado si está disponible
        try:
            from utils.improved_face_recognition import (
                extraer_embedding_mejorado,
                comparar_embeddings_mejorado,
            )
            ...
            for idx, emb_registrado in enumerate(ds.embeddings_empleados):
                similitud = comparar_embeddings_mejorado(
                    embedding_actual, emb_registrado
                )
            ...
            print(
                f"[DEBUG] Reconocimiento mejorado: reconocido={reconocido}, confianza={confianza:.3f}, idx={mejor_idx}, embeddings_disponibles={len(ds.embeddings_empleados)}"
            )
            ...
            print(
                f"[DEBUG] Reconocimiento: reconocido={reconocido}, confianza={confianza:.3f}, idx={idx}, embeddings_disponibles={len(ds.embeddings_empleados)}"
            )
            return reconocido, confianza, idx
