...
def check_supabase_sync():
    ...
    if not json_path.exists():
        print(f"⚠️ Manifiesto JSON no encontrado en {json_path}. Saltando validación de conteo.")
        return
    ...
    try:
        ...
        if local_count != cloud_count:
            print(f"❌ ERROR: Desincronización detectada ({local_count} vs {cloud_count}).")
            print("💡 Ejecuta 'python scripts/massive_cleanup.py' localmente para sincronizar.")
            sys.exit(1)
        else:
    ...
    try:
        ...
        test = sb.table("empleados").select("id, employee_id, embeddings").limit(1).execute()
