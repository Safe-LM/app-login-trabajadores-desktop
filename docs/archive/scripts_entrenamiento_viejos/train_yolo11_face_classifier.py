...
def train_yolo11_model(dataset_dir: Path, epochs: int = 100):
    ...
    try:
        ...
        if not use_val:
            print("⚠️ No hay imágenes en validación. Deshabilitando validación durante entrenamiento.")
            print("   (Esto es normal si solo hay 1 foto por empleado)")
            print()
        
        results = model.train(
            data=str(dataset_dir),  # Pasar el directorio directamente, no un archivo YAML
            epochs=epochs,
        ...
        possible_paths = [
            ...
            Path(__file__).parent / 'runs' / 'classify' / 'yolo_face_model' / 'face_classifier' / 'weights' / 'best.pt'
        ]
