...
def _check_dependencies():
    ...
    if _face_recognition_available is None:
        try:
            ...
            logger.info(
                f"face_recognition no disponible (opcional): {type(e).__name__}"
            )
...
class PhotoMatcher:
    ...
    def __init__(
        self, database_dir: Optional[Path] = None, lazy_load_dependencies: bool = True
    ):
        """
        ...
        if lazy_load_dependencies:
            ...
            self.torch_available, self.face_recognition_available = (
                _check_dependencies()
            )
        ...
        logger.info(
            f"✅ PhotoMatcher inicializado con {len(self.employees_db)} empleados"
        )
    ...
    def _preprocess_image(self, image: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            logger.debug(
                f"Error en preprocesamiento avanzado: {e}, usando método simple"
            )
    ...
    def _extract_face_features_opencv(
    ...
    def _extract_face_features_face_recognition(
        self, face_image: np.ndarray
    ) -> Optional[np.ndarray]:
        """
    ...
    def _get_employee_encoding(
        self, employee_id: int, method: str = "face_recognition"
    ) -> Optional[np.ndarray]:
        """
        ...
        employee = next(
            (e for e in self.employees_db if e.get("employee_id") == employee_id), None
        )
        if not employee:
            return None
        ...
        try:
            ...
            for employee in self.employees_db:
                ...
                for method_name, query_features in features_list:
                    ...
                    employee_encoding = self._get_employee_encoding(
                        employee_id, method_name
                    )
                    ...
                    similarity = self._compare_features(
                        query_features, employee_encoding
                    )
            ...
            if best_confidence >= min_confidence and best_employee:
                logger.info(
                    f"✅ Match encontrado: {best_employee.get('nombre')} (confianza: {best_confidence:.3f})"
                )
                return True, best_confidence, best_employee
            else:
                logger.debug(
                    f"No se encontró match (mejor confianza: {best_confidence:.3f})"
                )
            ...
            logger.error(
                f"❌ Error inesperado en match_photo_to_database: {type(e).__name__}: {e}"
            )
            import traceback
...
def get_photo_matcher(
    database_dir: Optional[Path] = None, lazy_load: bool = True
) -> Optional[PhotoMatcher]:
    """Obtener instancia global del matcher (con lazy load para evitar bloqueos)."""
    global _photo_matcher

    if _photo_matcher is None:
        try:
            _photo_matcher = PhotoMatcher(
                database_dir, lazy_load_dependencies=lazy_load
            )
        except Exception as e:
...
def match_photo_from_frame(
    frame: np.ndarray, min_confidence: float = 0.75, database_dir: Optional[Path] = None
) -> Tuple[bool, float, Optional[Dict]]:
    """
