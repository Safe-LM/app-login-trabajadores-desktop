...
end_point = None
current_image = None
current_window = "Selecciona la cara - Presiona ESPACIO para continuar, ESC para cancelar"
...
def mouse_callback(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if selecting:
            ...
            cv2.putText(img_copy, "Arrastra para seleccionar la cara", (10, 30),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            cv2.putText(img_copy, "ESPACIO: Continuar | ESC: Cancelar | R: Reiniciar", (10, 60),
        ...
        if start_point and end_point:
            ...
            if abs(x2 - x1) > 20 and abs(y2 - y1) > 20:  # Mínimo tamaño
                ...
                cv2.putText(img_copy, "Cara seleccionada! Presiona ESPACIO", (10, 30),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                cv2.putText(img_copy, "ESPACIO: Continuar | ESC: Cancelar | R: Reiniciar", (10, 60),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
                cv2.imshow(current_window, img_copy)

def select_face_interactive(img: np.ndarray, photo_name: str) -> Optional[Tuple[int, int, int, int]]:
    """
    ...
    try:
        ...
        if mejor_bbox and mejor_conf > 0.3:
            ...
            cv2.rectangle(display_img, (int(x1/scale_factor), int(y1/scale_factor)), 
                         (int(x2/scale_factor), int(y2/scale_factor)), (255, 0, 0), 2)
            cv2.putText(display_img, f"Auto-detectado (conf: {mejor_conf:.2f})", (10, 30),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
    except:
        pass
    
    # Mostrar instrucciones
    cv2.putText(display_img, f"Foto: {photo_name}", (10, display_img.shape[0] - 60),
               cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
    cv2.putText(display_img, "Arrastra para seleccionar la cara", (10, display_img.shape[0] - 30),
               cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
    cv2.putText(display_img, "ESPACIO: Usar seleccion | ESC: Cancelar | R: Reiniciar | A: Usar auto-detectado",
...
def prepare_training_data_interactive(database_fotos_dir: Path) -> Tuple[List[np.ndarray], List[int], Dict[int, Dict]]:
        ...
        for photo_file in photo_files:
            ...
            while True:
                ...
                if processed is not None:
                    ...
                    print(f"  ✅ Cara agregada (Total para este empleado: {employee_info[label_idx]['num_fotos']})")
                    
                    # Preguntar si quiere agregar otra cara de la misma foto
                    respuesta = input("  ¿Agregar otra cara de esta misma foto? (s/n): ").lower()
                    if respuesta != 's':
                        break
                else:
...
def train_model(images: List[np.ndarray], labels: List[int], num_classes: int, 
    ...
    X_train, X_val, y_train, y_val = train_test_split(
        images, labels, test_size=0.2, random_state=42, stratify=labels if len(set(labels)) > 1 else None
    )
...
def main():
    ...
    try:
        images, labels, employee_info = prepare_training_data_interactive(database_fotos_dir)
