...
def detect_and_crop_person_auto(
    img: np.ndarray, yolo_model
...
def organize_photos_by_employee(
    database_fotos_dir: Path, employee_data: Dict[int, Dict]
) -> Dict[int, List[Path]]:
    ...
    for employee_id, emp_info in sorted(employee_data.items()):
        ...
        if not photo_file:
            print(
                f"  [WARNING] Empleado {employee_id} ({emp_info.get('nombre', 'N/A')}): Sin photo_file en JSON"
            )
            continue
        ...
        for possible_name in possible_names:
            if possible_name in all_photo_files:
                ...
                print(
                    f"  [OK] Empleado {employee_id} ({emp_info.get('nombre', 'N/A')}): {photo_path.name}"
                )
                matched_count += 1
                found = True
                break
        ...
        if not found:
            print(
                f"  [WARNING] Empleado {employee_id} ({emp_info.get('nombre', 'N/A')}): Foto no encontrada ({photo_name})"
            )
    ...
    if matched_count < len(employee_data):
        ...
        for foto_file in sorted(unmatched_files):
            ...
            if match:
                ...
                if (
                    employee_id in employee_data
                    and employee_id not in photos_by_employee
                ):
                    photos_by_employee[employee_id] = [foto_file]
                    print(
                        f"  [OK] Empleado {employee_id} emparejado por nombre: {foto_file.name}"
                    )
                    matched_count += 1
    ...
    if not photos_by_employee:
        raise ValueError(
            "[ERROR] No se encontraron fotos para ningún empleado. Verifica que las fotos existan y que el JSON tenga los campos 'photo_file' correctos."
        )

    return photos_by_employee


def prepare_training_data_auto(
    database_fotos_dir: Path, yolo_model
    ...
    for employee_id, photo_files in sorted(photos_by_employee.items()):
        ...
        print(
            f"  Zona: {employee_info[label_idx]['zona']} | Sucursal: {employee_info[label_idx]['sucursal']} | Puesto: {employee_info[label_idx]['puesto']}"
        )
        ...
        for photo_file in photo_files:
            try:
                ...
                if conf < 0.25:
                    print(
                        f"  [WARNING] {photo_file.name}: Confianza YOLO baja ({conf:.2f}), usando imagen completa"
                    )
                elif conf < 0.5:
                    print(
                        f"  [INFO] {photo_file.name}: Confianza YOLO media ({conf:.2f})"
                    )
                ...
                if processed is not None:
                    ...
                    print(
                        f"  [OK] {photo_file.name} procesada (confianza YOLO: {conf:.2f})"
                    )
                else:
        ...
        print(
            f"  Total caras agregadas para este empleado: {employee_info[label_idx]['num_fotos']}\n"
        )
...
def train_model(
    ...
    print(
        f"  Clases con 1 muestra: {sum(1 for count in label_counts.values() if count == 1)}"
    )

    # Manejar caso especial: todas las clases tienen solo 1 muestra
    if min_samples_per_class == 1 and max_samples_per_class == 1:
        print(
            f"[INFO] Todas las clases tienen solo 1 muestra. Duplicando imágenes para crear conjunto de validación."
        )
        ...
        X_val = (
            images.copy()
        )  # Usaremos las mismas imágenes pero con transformaciones de validación
        y_val = labels.copy()
        print(
            f"[INFO] Usando todas las {len(images)} imágenes para entrenamiento y validación (duplicadas)"
        )
    else:
        ...
        if min_samples_per_class == 1:
            ...
            print(
                f"[INFO] Usando test_size=0.15 (algunas clases tienen solo 2 muestras)"
            )
        else:
        ...
        if not use_stratify:
            print(
                f"[WARNING] Usando split sin estratificación (algunas clases tienen solo 1 muestra)"
            )
        ...
        min_val_samples = max(
            1, int(num_classes * 0.1)
        )  # Al menos 10% de las clases o 1 muestra
        if test_size * len(images) < min_val_samples:
            ...
            print(
                f"[INFO] Ajustando test_size a {test_size:.2f} para asegurar al menos {min_val_samples} muestras en validación"
            )
        ...
        if len(X_train) == 0 or len(X_val) == 0:
            ...
            print(
                "[WARNING] Validación vacía después del split, duplicando imágenes para validación"
            )
            X_train = images.copy()
            y_train = labels.copy()
            X_val = images.copy()
    ...
    )  # Reducido de 0.1 a 0.05 para más precisión
    optimizer = optim.AdamW(
        model.parameters(), lr=learning_rate, weight_decay=0.005
    )  # Weight decay reducido
    # Scheduler con cosine annealing mejorado
    scheduler = optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=epochs, eta_min=learning_rate * 0.001
    )
        ...
        if val_loader is not None and len(X_val) > 0:
            ...
            if val_acc > best_val_acc:
                ...
                if patience_counter >= patience:
                    print(
                        f"  [INFO] Early stopping: sin mejora por {patience} epochs. Mejor Val Acc: {best_val_acc:.2f}%"
                    )
                    break
        else:
            print(
            ...
            if train_acc > best_val_acc:
                ...
                patience_counter = 0
                print(
                    f"  [OK] Nuevo mejor modelo guardado (Train Acc: {train_acc:.2f}%)"
                )
            else:
                patience_counter += 1
                if patience_counter >= patience:
                    print(
                        f"  [INFO] Early stopping: sin mejora por {patience} epochs. Mejor Train Acc: {best_val_acc:.2f}%"
                    )
                    break
...
def main():
    ...
    try:
        ...
        images, labels, employee_info = prepare_training_data_auto(
            database_fotos_dir, yolo_model
        )
        ...
        model = train_model(
            images, labels, num_classes, epochs=100, batch_size=8, learning_rate=0.0003
        )
