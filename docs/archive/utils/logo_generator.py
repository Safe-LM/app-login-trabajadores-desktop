...
def load_logo_from_file(size=200):
    ...
    logo_paths = [
        ...
        / "app_loginTrabajadores_desktop_pyqt"
        / "logo_empresa.png",  # Ruta alternativa
    ]
...
def create_safelink_logo(size=200):
    ...
    shield_gradient = QLinearGradient(
        shield_x, shield_y, shield_x + shield_size, shield_y + shield_size
    )
    ...
    shield_points = [
        ...
        QPoint(
            int(shield_x + shield_size), int(shield_y + shield_size * 0.7)
        ),  # Bottom right
        QPoint(
            int(shield_x + shield_size), int(shield_y + shield_size * 0.2)
        ),  # Top right
    ]
    ...
    # Cuerpo del candado
    painter.drawRoundedRect(
        lock_x, int(lock_y + lock_size * 0.2), lock_size, int(lock_size * 0.6), 2, 2
    )
    ...
    circuit_lines = [
        # Líneas superiores
        (
            center_x - shield_size * 0.6,
            center_y - shield_size * 0.5,
            center_x - shield_size * 0.3,
            center_y - shield_size * 0.5,
        ),
        (
            center_x + shield_size * 0.3,
...
def create_banner_logo(width=400, height=100):
    ...
    painter.drawText(
        QRect(100, 10, width - 110, 50), Qt.AlignLeft | Qt.AlignVCenter, "SAFE LINK"
    )
    ...
    painter.drawText(
        QRect(100, 50, width - 110, 40), Qt.AlignLeft | Qt.AlignVCenter, "SECURITY"
    )
