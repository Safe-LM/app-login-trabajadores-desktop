...
class HybridOpenCVGeminiMatcher:
    ...
    def match_photo(
        ...
        if self._check_opencv_lazy() and self.opencv_matcher:
            try:
                matched, confidence, employee_info = self.opencv_matcher.recognize(
                    frame
                )
                if matched and employee_info and confidence >= min_confidence:
                    logger.info(
                        f"Match OpenCV: {employee_info.get('nombre', 'Unknown')} ({confidence:.1%})"
                    )
        ...
        if prefer_gemini and self._check_gemini_lazy() and self.gemini_matcher:
            try:
                ...
                if matched and employee_info:
                    logger.info(
                        f"Match Gemini: {employee_info.get('nombre', 'Unknown')} ({confidence:.1%})"
                    )
...
def get_hybrid_matcher(
    database_dir: Optional[Path] = None,
) -> Optional[HybridOpenCVGeminiMatcher]:
    """
