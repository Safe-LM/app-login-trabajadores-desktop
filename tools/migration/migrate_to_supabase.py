...
sys.path.append(str(root_dir / "station" / "src"))
...
def migrate_employees():
    ...
    logger.info(
        f"Encontrados {len(trabajadores)} trabajadores en la base de datos local."
    )
    ...
    for t in trabajadores:
        ...
        if encs_data:
            indices = [
                i
                for i, eid in enumerate(encs_data.get("employee_ids", []))
                if eid == t.employee_id
            ]
            if indices:
        ...
        try:
            ...
            if employee_embs and any(np.isnan(employee_embs)):
                logger.warning(
                    f"⚠️ Embeddings de {t.nombre} contienen NaN, se enviarán vacíos."
                )
                employee_embs = []

            # Upsert por employee_id
            res = (
                sb.table("empleados").upsert(data, on_conflict="employee_id").execute()
            )
