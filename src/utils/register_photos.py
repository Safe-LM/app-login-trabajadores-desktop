...
def register_photos_from_database():
    ...
    try:
        ...
        for foto_file in sorted(foto_files):
            try:
                ...
                if mejor_bbox is None:
                    ...
                    print(
                        f"  ⚠️ No se detectó persona con YOLO, usando imagen completa"
                    )
                else:
                ...
                embedding_type = (
                    "OSNet" if ds.modelo_osnet is not None else "Histograma HSV"
                )
                embedding_size = (
                    len(embedding) if hasattr(embedding, "__len__") else "N/A"
                )
                print(
                    f"  📊 Embedding extraído: Tipo={embedding_type}, Tamaño={embedding_size}"
                )
                ...
                print(
                    f"  ✅ Registrado: embedding_idx={idx}, employee_id={employee_id}"
                )
                print()
                registered_count += 1
        ...
        if registered_count > 0:
            ...
            print(
                f"💡 Ahora el sistema puede reconocer a estos {registered_count} trabajadores"
            )
            return True
        else:
