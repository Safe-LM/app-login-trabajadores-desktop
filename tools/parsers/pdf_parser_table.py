...
def parse_table_pdf_text(pdf_text: str) -> List[Dict]:
    ...
    for i, line in enumerate(lines):
        ...
        if (
            "ZONA" in line_upper
            and "SUCURSAL" in line_upper
            and "PUESTO" in line_upper
            and "NOMBRE" in line_upper
        ):
            in_table = True
            header_found = True
        ...
        if in_table and not skip_next:
            ...
            if line and not ("ZONA" in line.upper() and "SUCURSAL" in line.upper()):
                ...
                if rows_processed <= 5:
                    print(
                        f"    Fila {rows_processed}: {len(parts)} columnas - {line[:80]}"
                    )
                ...
                if len(parts) >= 4:
                    ...
                    if (
                        ...
                        and zona != "#VALUE!"
                        and len(nombre) > 1
                        and nombre.upper() not in ["FOTO", "NOMBRE"]
                    ):
                        ...
                        employees.append(
                            {
                                ...
                                "zona": zona,
                                "sucursal": (
                                    sucursal if sucursal else zona
                                ),  # Si no hay sucursal, usar zona
                                "puesto": puesto,
                            }
                        )

                        print(
                            f"    Empleado {employee_id}: {nombre} - {puesto} ({zona}, {sucursal})"
                        )
                        employee_id += 1
...
def extract_from_table_pdf_file(pdf_path: Path) -> List[Dict]:
    ...
    print(
        "[ERROR] Todos los metodos de extraccion fallaron o no se encontraron empleados"
    )
    return []
...
def create_csv_from_table_pdf(pdf_path: Path, output_csv_path: Path) -> bool:
    ...
    for emp in employees[:5]:
        print(
            f"     {emp['employee_id']}: {emp['nombre']} - {emp['puesto']} ({emp['zona']}, {emp['sucursal']})"
        )
